...
export default function PricingBreakdown({ pricing, paymentAmount }) {
    ...
    const { baggagePrice, mealPrice } = useBookingStore(
        (state) => state.addOns
    );
    const { premium: insurancePrice } = useBookingStore(
        (state) => state.selectedInsurance
    );
    ...
    return (
        <div className="space-y-4 border border-gray-200 dark:border-gray-700 flex-1 py-4  px-6 rounded-2xl">
