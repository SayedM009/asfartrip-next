...
export default function FlightStatusPage({
    ...
    try {
        if (bookingData?.BookingData?.payment_success) {
            paymentSuccessData = JSON.parse(
                bookingData.BookingData.payment_success
            );
        }
    } catch (error) {
    ...
    if (bookingData?.BookingData?.response?.onward) {
        ...
        const onwardSegments =
            bookingData.BookingData.response.onward.segments || [];
        const returnSegments =
            bookingData.BookingData.response.return?.segments || [];
    ...
    const baggageAllowance =
        bookingData?.BookingData?.response?.BaggageAllowance?.[0];
    ...
    const luggageTotal = parseFloat(
        bookingData?.BookingData?.travelfusion_luggage_total || 0
    );
    const insurancePremium = parseFloat(
        bookingData?.BookingData?.insurance_premium || 0
    );
    ...
    const cardType =
        paymentInfo?.card_type || paymentSuccessData?.card?.type || null;

    const cardLast4 =
        paymentInfo?.card_last4 || paymentSuccessData?.card?.last4 || null;

    const transactionDate =
        paymentInfo?.transaction_date ||
    ...
    const booking = {
        ...
        contact: {
            name: `${bookingData?.BookingData?.GUEST_FIRSTNAME || ""} ${
                bookingData?.BookingData?.GUEST_LASTNAME || ""
            }`.trim(),
        ...
        payment: {
            ...
            method:
                cardType || bookingData?.BookingData?.payment_method || "N/A",
    ...
    return (
        <div className="relative min-h-screen overflow-hidden bg-gradient-to-b my-6 md:my-12">
            ...
            <div className="relative z-10 container mx-auto ">
                {/* Status Hero */}
                <StatusHero status={status} bookingRef={bookingRef} pnr={pnr} />
